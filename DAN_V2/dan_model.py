...
class Model(object):
    ...
    def __init__(self,
        ...
        self.__pixels__ = tf.constant(
                [(x, y) for y in range(self.img_size) for x in range(self.img_size)],
    ...
    def __calc_affine_params(self, from_shape, to_shape):
        ...
        norm = tf.norm(from_centralized, axis=[1, 2])
        norm_pow_2 = tf.pow(norm, 2)
        ...
        b = tf.reduce_sum(
            tf.multiply(
                from_centralized[:, :, 0],
                to_centralized[:, :, 1]) - tf.multiply(from_centralized[:, :, 1],
                to_centralized[:, :, 0]
            ),
            1
        ) / norm_pow_2
    ...
    def __affine_image(self, imgs, r, t):
        ...
        rm = tf.reshape(
            tf.pad(r, [[0, 0], [0, 0], [0, 1]], mode='CONSTANT'),
            [-1, 6]
        )
    ...
    def __call__(
        ...
        with tf.variable_scope('s1'):
            ...
            for i, num_filter in enumerate(self.filter_sizes):
                inputs = vgg_block(
                        inputs=inputs, filters=num_filter, num_convs=self.num_convs,
...
class MulEfficient(Module):

    def __init__(self, efficient):
        super(MulEfficient, self).__init__()
        self.efficient = efficient

    def forward(self, input):       
        return input.matmul(self.efficient)
